...
try {
    $stmt = $pdo->prepare("SELECT * FROM webbooks WHERE id = 6");
    $stmt->execute();
    $book = $stmt->fetch(PDO::FETCH_ASSOC);

    if (!$book) {
        die("Data Grounding Fault: WebBook ID 6 not found in MariaDB.");
    }
    ...
    <div class="max-w-4xl w-full bg-white rounded-[2.5rem] gac-shadow overflow-hidden mb-10">
        ...
        <div class="p-8 md:p-14 text-center border-b border-gray-100">
            <h1 class="text-[11px] font-black uppercase tracking-[0.4em] text-emerald-600 mb-3">
                <?php echo htmlspecialchars($book['category'] ?? 'AI PRESENTATION FOUNDRY'); ?>
            </h1>
