...
if (!$student_id || !$selected_course) {
    header("Location: exam-fin-sel.php?error=session_loss");
    exit();
}
$course_code = trim($selected_course); 
//GAC Infrastructure Fault: SQLSTATE[HY093]: Invalid parameter number
try {
    // 2. INITIALIZATION: Clear the temporary answers table
    $pdo->exec("TRUNCATE TABLE gac_exam_answers");
	
    if (strtoupper(substr($course_code, 0, 3)) === 'CER') {
		 $stmt = $pdo->prepare("
            SELECT * FROM gac_exam_questions 
            WHERE CAST(level AS UNSIGNED) < 11 
            ORDER BY RAND() 
            LIMIT 50
        ");
        $stmt->execute();
...
<head>
    ...
    <title>AIGC | FINAL Exam - <?php echo htmlspecialchars($selected_class); ?></title>
...
<body class="bg-gray-50 min-h-screen p-2 md:p-6 font-sans">
    ...
    <div class="max-w-4xl mx-auto gac-frame bg-white rounded-3xl shadow-2xl overflow-hidden">
        <header class="bg-[#064e3b] text-white p-8 text-center">
            <h1 class="text-3xl font-black uppercase tracking-tighter">AIGC FINAL Examination </h1>
        ...
        <form id="examForm" action="exam-fin-result.php" method="POST" class="p-6 md:p-10">
