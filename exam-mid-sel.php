...
$stmt = $pdo->prepare("SELECT course_code, marks_mid, marks_final FROM course_regit WHERE student_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['typed_course'])) {
    ...
    header("Location:exam-mid-list.php");
    exit();
}
?>
...
<body class="min-h-screen p-8 text-white">
    ...
    <div class="max-w-4xl mx-auto">
        <header class="text-center mb-10">
            <h1 class="text-4xl font-black uppercase  tracking-tighter">AIGC Mid Term Exam Selection</h1>
            ...
            <table class="w-full text-left">
                ...
                <tbody class="divide-y divide-white/5">
                    ...
                    <tr class="font-bold text-sm">
                        <td class="py-4"><?php echo htmlspecialchars($c['course_code']); ?></td>
        ...
        <div class="glass-card rounded-[2rem] p-10">
            ...
            <form method="POST" class="space-y-8"><form id="examForm" action="exam-mid-list.php" method="POST" class="p-6 md:p-10"> 
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        <label class="block text-[10px] font-black uppercase tracking-widest text-emerald-500 mb-3">Type Exam Term (Mid) in</label>
                        <input type="text" name="typed_exam_term" required placeholder="Mid"
                               class="w-full bg-black/40 border border-white/20 rounded-xl px-6 py-4 text-white font-bold focus:border-white-500 outline-none uppercase">
                    </div>
                </div>

                <div class="pt-4">
                    <button type="submit" class="w-full bg-red-600 hover:bg-emerald-500 text-slate-950 font-black py-5 rounded-2xl uppercase tracking-widest text-sm transition-all shadow-xl transform hover:-translate-y-1">
                        Confirm Selection & Proceed to Exam
                    </button>
                </div>
            </form>
        </div>
